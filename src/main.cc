...
// Globalize buffers? So that they maintain state and end up flushing themselves
...
int main(int argc, char **argv)
{
	...
	{
		transform.reset(new BinToAsciiTransform(in, out));
	}
	else if (args.it == Type::ASCII && args.ot == Type::BIN)
	{
		transform.reset(new AsciiToBinTransform(in, out));
	}
